function [F] = mrsba_opter(x ,cam_fir,pram, camera_set)
 ...
 %--------------------------------------------------------------------------   
    for i=1:num_camera 
        for j = 1:num_points
            ...
            u_obs = obs(1);
            v_obs = obs(2);
            F(1:2, measurement_id) = error_obs(X,Y,Z,d_x,d_y,d_z,fx,fy,rx,ry,rz,tx,ty,tz,u0,u_obs,v0,v_obs,vi,w_x,w_y,w_z);
