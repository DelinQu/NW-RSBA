function [F,J] = wmrsba_opter(x,cam_fir,pram, camera_set)
 ...
 %--------------------------------------------------------------------------   
    for i=1:num_camera 
        for j = 1:num_points
            ...
            F(1:2, measurement_id) = error_obs_weighted_opt(X,Y,Z,d_x,d_y,d_z,fx,fy,rx,ry,rz,tx,ty,tz,u0,u_obs,v0,v_obs,vi,w_x,w_y,w_z);

            if i ~= 1
                tmp_j = J_weighted_opt(X,Y,Z,d_x,d_y,d_z,fx,fy,rx,ry,rz,tx,ty,tz,u0,v0,v_obs,vi,w_x,w_y,w_z);
